...
const Logo = () => {
  return (
    <Link className="flex items-center text-white" to={APP_PATH.home.getHref()}>
      ...
      <span className="text-sm font-semibold text-white">F-Library</span>
    </Link>
  );
};
...
export function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const logout = useLogout({
    onSuccess: () => navigate(APP_PATH.auth.login.getHref(location.pathname)),
  });
  const { checkAccess } = useAuthorization();
  const navigation = [
    { name: 'Dashboard', to: APP_PATH.app.dashboard.getHref(), Icon: Home },
    { name: 'Discussions', to: APP_PATH.app.discussions.getHref(), Icon: Folder },
    checkAccess({ allowedRoles: [ROLES.ADMIN] }) && {
      name: 'Users',
      to: APP_PATH.app.users.getHref(),
      Icon: Users,
    },
        ...
        <header className="sticky top-0 z-30 flex h-14 items-center justify-between gap-4 border-b bg-background px-4 sm:static sm:h-auto sm:justify-end sm:border-0 sm:bg-transparent sm:px-6">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem
                onClick={() => navigate(APP_PATH.app.profile.getHref())}
